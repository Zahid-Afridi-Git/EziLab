...
export function ProjectCard({ project, className }: ProjectCardProps) {
  return (
    <article className={cn("card card-glow group overflow-hidden", className)}>
      ...
      <div className="space-y-4 p-5">
        <div className="flex flex-wrap items-center gap-2 text-xs text-muted">
          <span className="inline-flex rounded-full bg-brand-cyan/10 px-2.5 py-1 font-medium text-brand-cyan">{project.category}</span>
        ...
        <div>
          <Link href={`/projects/${project.slug}`}>
            <h3 className="font-heading text-lg font-semibold text-white transition group-hover:text-brand-cyan">{project.title}</h3>
          </Link>
